...
export function TaskCard({ task }: { task: Task }) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`focus-ring touch-none ${isDragging ? "z-50 cursor-grabbing" : "cursor-grab"}`}
      style={{ transform: CSS.Translate.toString(transform), position: "relative" }}
    >
      <motion.article
        layoutId={task.id}
        layout
        ...
        className="relative overflow-visible rounded-xl border bg-[rgba(16,22,31,0.85)] p-3 backdrop-blur-sm"
        style={{
          borderColor: isDragging ? `${def.color}88` : `${def.color}30`,
          boxShadow: isDragging
            ? `0 18px 40px rgba(0,0,0,0.55), 0 0 22px ${def.color}33`
